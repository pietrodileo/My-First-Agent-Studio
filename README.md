...
- Click the large top-left logo to collapse or expand the sidebar.
...
- Responses support Markdown, including GFM tables and code blocks. Wide tables
...
1. Select `Test.MathAgent` and an installed model in the UI. Enable its
   calculation-review skill, then ask: “Call add_numbers with first=17 and
...
4. Run both terminal demos and the manual session to explore skill activation
...
![SimpleAgent chat with Ollama, sidebar controls, and a Markdown response table](pic/image_1.png)
...
![Ollama model dropdown showing multiple local models for Test.Agent](pic/model_choice.png)
...
![Selected agent skill controls showing Caveman on, Echo off, and Ollama connected](pic/image_2.png)
